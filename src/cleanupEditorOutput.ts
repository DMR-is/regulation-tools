...
import { makeMutators } from './_cleanup/cleanup-utils';
import {
 ...
 */
export const cleanupSingleEditorOutput = (html: HTMLText): HTMLText => {
...
export const cleanupAllEditorOutputs = ({
  ...
  text: cleanupSingleEditorOutput(text),
  appendixes: appendixes.map(({ title, text }) => ({
    title: cleanTitle(title),
    text: cleanupSingleEditorOutput(text),
  })),
  comments: cleanupSingleEditorOutput(comments),
});
...
export const cleanupAndCombineEditorOutputs = (
  ...
  return prettify(cleanCombined) as HTMLText;
};
